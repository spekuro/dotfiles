...
globalkeys = my_table.join(
    -- Take a screenshot
--		awful.key({ }, "Print", function () awful.util.spawn("scrot '%Y-%m-%d_$wx$h_scrot.png' -e 'mv $f ~/Images/screenshots/'", false) end,
    awful.key({ }, "Print", function () awful.spawn(screenprint) end,
...
--[[
    awful.key({ altkey, "Control" }, "m",
        function ()
            os.execute(string.format("amixer -q -D pulse set %s 100%%", beautiful.volume.channel))
            beautiful.volume.update()
        end,
        {description = "volume 100%", group = "hotkeys"}),
    awful.key({ altkey, "Control" }, "0",
        function ()
            os.execute(string.format("amixer -q -D pulse set %s 0%%", beautiful.volume.channel))
            beautiful.volume.update()
        end,
